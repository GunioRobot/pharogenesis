fontName: fontName size: ptSize emphasis: emphasisCode rangesArray: rangesArray 
	...
	ranges := rangesArray.
	ranges ifNil: [ ranges := Array with: (Array 
				with: 0
				with: 255) ].
	pointSize := ptSize.
	name := fontName.
	...
	xStart := maxWidth := 0.
	rangesStream := (ranges collect: [ :e | e first to: e second ]) readStream.
	...
	0 
		...
		do: 
			...
			i > currentRange last ifTrue: 
				[ 
				[ rangesStream atEnd not and: 
					[ currentRange := rangesStream next.
					currentRange last < i ] ] whileTrue.
