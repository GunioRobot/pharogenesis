profileHintedDisplayCachedUsingMode34
	"
	self profileHintedDisplayCachedUsingMode34
	"
	| t f m text canvas |
	Preferences enable: #HintingNone.
	Preferences enable: #HintingLight. "cache is now clear"
...
We know who you are!
	...
	stephane.ducasse@free.fr and denker@iam.unibe.ch' asText.
text addAttribute: (TextFontReference toFont: f).
m contents: text.
m openInWorld.
FreeTypeSettings current pretendBitBltSubPixelUnavailableDuring:[
	canvas := (FormCanvas on: (Form extent: 1000@1000 depth: 32)).
	m drawOn: canvas. "this fills the cache"
