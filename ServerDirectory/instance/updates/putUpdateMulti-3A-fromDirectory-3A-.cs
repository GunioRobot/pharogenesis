putUpdateMulti: list fromDirectory: updateDirectory 
	...
	(self checkNames: (list collect: 
			[ :each | 
			"Check the names without their numbers"
			each 
				copyFrom: (each findFirst: [ :c | c isDigit not ])
				to: each size ])) ifFalse: [ ^ nil ].
	response := UIManager default 
		chooseFrom: #('Install update' 'Cancel update' )
		title: 'Do you really want to broadcast ' , list size printString , ' updates' , '\to every Squeak user who updates from ' withCRs , self groupName , '?'.
	...
	versIndex < listContents size ifTrue: 
		[ response := UIManager default 
			chooseFrom: #(
					'Make update for an older version'
					'Cancel update'
				)
		...
		numStr ifNil: [numStr := String new].
	...
	newNames := list 
		with: (lastNum + 1 to: lastNum + list size)
		collect: 
			[ :each :num | 
			seq := num printString 
				padded: #left
	...
	listContents 
		...
		put: { 
				version.
				((listContents at: versIndex) second , newNames)
			 }.
