markMatchingClasses
	...
	receiverString := (selectorList at: selectorIndex) readStream upToAll: firstPartOfSelector.
	receiver := Compiler evaluate: receiverString.
	unmarkedClassList do: 
		[ :classAndMethod | 
		| class |
		(classAndMethod isKindOf: MethodReference) 
			ifTrue: 
				[ (receiver isKindOf: classAndMethod actualClass) ifTrue: [ classAndMethod stringVersion: '*' , classAndMethod stringVersion ] ]
			ifFalse: 
				[ class := Compiler evaluate: (classAndMethod readStream upToAll: firstPartOfSelector).
