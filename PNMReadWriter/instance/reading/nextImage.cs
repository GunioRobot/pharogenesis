nextImage
	...
	first 
		...
		ifNotNil: 
			[ type < 4 ifTrue: [ self error: 'Plain PBM, PGM or PPM have only one image' ] ].
	stream ascii.
	p := stream next.
	type := stream next asInteger - 48.
	(p = $P and: [ type > 0 and: [ type < 8 ] ]) ifFalse: [ self error: 'Not a PNM file' ].
	type = 7 
		ifTrue: [ self readHeaderPAM ]
		ifFalse: [ self readHeader ].
	type caseOf: { 
			([ 1 ] -> [ ^ self readPlainBW ]).
