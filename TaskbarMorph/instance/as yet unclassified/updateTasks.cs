updateTasks
	...
	newTasks copy do: [:t |
		(self orderedTasks detect: [:ot | ot morph = t morph] ifNone: []) ifNotNilDo: [:ot |
	...
	WorldState addDeferredUIMessage: [self layoutChanged] "may have a different number of rows"