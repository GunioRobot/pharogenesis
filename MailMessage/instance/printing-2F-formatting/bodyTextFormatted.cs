	...
	self body isMultipart ifTrue: 
		...
		self body isMultipartAlternative ifTrue: 
			...
			#('text/html' 'text/plain' ) do: 
				[ :format | 
				self parts do: [ :part | part body contentType = format ifTrue: [ ^ part bodyTextFormatted ] ] ].
			...
			self parts do: 
				...
				((#('text' 'multipart' ) includes: part body mainType) or: [ part body contentType = 'message/rfc822' ]) 
					...
					ifFalse: 
						...
						str nextPutAll: (Text 
								string: '[' , descript , ']'
								attribute: (TextMessageLink message: part)) ] ] ] ].
	...
	self body contentType = 'text/html' ifTrue: 
		[ Smalltalk 
			...
			ifPresentAndInMemory: 
				[ :htmlParser | 
				^ (htmlParser parse: body content readStream) formattedText ] ].

	"check for an embedded message"
	self body contentType = 'message/rfc822' ifTrue: [ ^ (MailMessage from: self body content) formattedText ].
